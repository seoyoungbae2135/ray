...
// document.write("평균치 : "+avg+"<br><br>");
// document.write("평균치이상<br>");
...
// 2023.11.23 homework 해설

const name = ["주현상","윤대경","이민우","이태양","페냐","박상원","장시환"]; //실수로 데이터 변경되지 않도록.
const era = [1.96, 2.45, 2.63, 3.23, 3.60, 3.65, 3.38]; // 또는 다른 개발자에게 변경하지 않도록 알리는 방법으로 const사용

var total = 0; //평균자책점(era)의 총점을 구하기위한 누적변수
for( var i=0; i<era.length; i++){
    total += era[i]; // total = total + era[i] 평균자책점 총점구하기
}
var avg = total/era.length; //평균 구하기

for(var i=0; i<name.length; i++){
    if( avg <= era[i]){  //평균이상인 값 찾기
